...
namespace rec_rewild_classic.api
{
    class ProfieStealer
    {
        ...
        public static bool Profilefind(string data, int skip = 0, int take = 10)
        {
            ...
            List<Root> profiles = JsonConvert.DeserializeObject<List<Root>>(data);
            ...
            if (take > totalProfiles) take = totalProfiles; // Ensuring take isn't greater than available profiles.
            int maxPage = (totalProfiles + take - 1) / take;
            string input;
            string error_txt = "";
            while (true)
            {
                Console.Clear();
                if (!string.IsNullOrEmpty(error_txt))
                    Console.WriteLine();
                error_txt = "";
                Console.WriteLine($"{BLUE}┌────────────────────────────┤ {UNDERLINE}{NORMAL}Player Profiles{NOUNDERLINE} {BLUE}├────────────────────────────┐");

                int currentPage = (skip / take) + 1;

                for (int i = skip; i < Math.Min(skip + take, totalProfiles); i++)
                {
                    Console.WriteLine($"{BLUE}│ {NORMAL}[{i}] {profiles[i].displayName}  @{profiles[i].username}".PadRight(86) + $"{BLUE}│");
                }

                Console.WriteLine($"{BLUE}├──────────" + (currentPage > 1 ? $"┤ {NORMAL}{UNDERLINE}Prev{NOUNDERLINE} {BLUE}├" : "────────") +
                                  $"────────────┤{NORMAL} {UNDERLINE}Exit{NOUNDERLINE} {BLUE}├────────────" +
                                  (currentPage < maxPage ? $"┤ {UNDERLINE}{NORMAL}Next{NOUNDERLINE} {BLUE}├" : "────────") + "─────────────────┘");
                Console.WriteLine($"{BLUE}└──────────────────────────────┘ {NORMAL}Page {currentPage} / {maxPage}");
                ...
                else if (input == "N" && currentPage < maxPage)
                {
                    skip += take;
                }
                else if (int.TryParse(input, out int value) && value >= 0 && value < totalProfiles)
                {
                    Console.WriteLine($"You selected: {profiles[value].displayName} (@{profiles[value].username})");
                    ProfileSteal(data, value);
                    return true;
                }
                else
                {
                    error_txt = $"'{input}' is not a valid selection. Please try again.";
                }
            }
        }
        /// <summary>
